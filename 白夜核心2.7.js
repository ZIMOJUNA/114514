...
if (!window.__PM_SEVEN_DAYS_MOUNTED__) {
	...
	let e = /* @__PURE__ */ new Map(), t = null, n = () => {
		...
		i.forEach((e) => {
			...
			if (i === -1) return;
			let a = n.textContent || "", o = n.getAttribute("is_user") === "true", s = a.includes("<initvar>") || a.includes("pm-custom-char-mount-point");
			if (a.includes("pm-custom-char-mount-point") && !n.hasAttribute("data-setup-mounted")) {
				n.setAttribute("data-setup-mounted", "true");
				let a = `pm-setup-root-${i}`;
				if (!r.getElementById(a)) {
					let n = r.createElement("div");
					n.id = a, r.body.appendChild(n), t = Yo(ip), t.mount(n), e.set(`setup_${i}`, t);
				}
			}
			if (!o && !s) {
				let t = n.querySelector(".pm-status-bar-container"), a = n.querySelector(".mes_block");
				if (a && !t && !e.has(i)) {
					t = r.createElement("div"), t.className = "pm-status-bar-container", t.id = `pm-app-${i}`, t.style.cssText = "margin-top: 15px; width: 100%; clear: both;", a.appendChild(t);
					let n = Yo(Wl, { mesId: i });
					n.mount(t), e.set(i, n);
				}
			}
		...
		Qo.isReady = !0, console.log("[SevenDays] 🚀 观测者终端已连接 (DOM-Driven)"), setInterval(() => {
			if (Qo.isReady) {
				for (let t of e.keys()) if (typeof t == "number") {
					let e = ts(t), n = Qo.mvuDataMap.get(t);
					JSON.stringify(e) !== JSON.stringify(n) && Qo.mvuDataMap.set(t, e);
				}
			}
		}, 1e3);
		let r = !1, o = !1, s = () => {
