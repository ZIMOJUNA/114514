...
if (!window.__PM_SEVEN_DAYS_MOUNTED__) {
	...
	let e = /* @__PURE__ */ new Map(), t = null, n = () => {
		...
		i.forEach((e) => {
			...
			if (i === -1) return;
			let a = n.textContent || "", o = n.getAttribute("is_user") === "true", s = a.includes("<initvar>") || a.includes("pm-custom-char-mount-point"), c = n.querySelector(".mes_block");
			if (a.includes("pm-custom-char-mount-point") && !n.hasAttribute("data-setup-mounted")) {
				...
				if (c && !r.getElementById(a)) {
					let n = r.createElement("div");
					n.id = a, n.style.cssText = "margin-top: 15px; width: 100%; position: relative; clear: both;", c.appendChild(n), t = Yo(ip), t.mount(n), e.set(`setup_${i}`, t);
				}
			}
			if (!o && !s) {
				let t = n.querySelector(".pm-status-bar-container");
				if (c && !t && !e.has(i)) {
					t = r.createElement("div"), t.className = "pm-status-bar-container", t.id = `pm-app-${i}`, t.style.cssText = "margin-top: 15px; width: 100%; clear: both;", c.appendChild(t);
					let n = Yo(Wl, { mesId: i });
					n.mount(t), e.set(i, n);
				}
			}
