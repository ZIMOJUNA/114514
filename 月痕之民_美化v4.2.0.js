...
function czStrip(g){
  ...
  return '<div class="cz' + (g.win ? ' ncol' : '') + '"'
    ...
    + (gi >= 0
        ? (next ? '<span class="nx">下一档<b>' + esc(next) + '</b></span>'
                : '<span class="nx">到头了</span>')
        : '')
    + '<span class="cmp">' + (g.cmp || '') + '</span>'
    + '<span class="rs">' + esc(g.verdict || '') + '</span></div>';
}
...
function parseReq(rec){
  ...
  if (m){
    var v = numsOf(m[1]);
    if (v.length >= 3){ o.roll.d = v[0]; o.roll.mod = v[1]; o.roll.tot = v[2]; }
  }
...
function reqPane(c){
  return '<details class="ck" open data-stage="before"' + tierAttr(c.gears[c.gi]) + '>'
    ...
    + czStrip({gears:c.gears, gi:c.gi,
        cmp:'<b>' + esc(c.roll.tot) + '</b><em>&gt;=</em><b>' + esc(c.dc) + '</b>',
        verdict:c.verdict})
    + '</details>';
}
...
function parseClash(rec){
  ...
  if (m){ var va = numsOf(m[1]); if (va.length >= 3){ o.a.d = va[0]; o.a.mod = va[1]; o.a.tot = va[2]; } }
  m = pick(rec, /^防方检定\s*[:：]\s*([\s\S]*)$/);
  if (m){ var vb = numsOf(m[1]); if (vb.length >= 3){ o.b.d = vb[0]; o.b.mod = vb[1]; o.b.tot = vb[2]; } }
...
function clashPane(c){
  ...
  return '<details class="ck" open'
    ...
    + czStrip({gears:c.gears, gi:c.gi, win:w,
        cmp:'<b>' + esc(c.a.tot) + '</b><em>&gt;</em><b>' + esc(c.b.tot) + '</b>',
        verdict:c.verdict})
    + '</details>';
}
