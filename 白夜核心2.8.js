...
if (!window.__PM_SEVEN_DAYS_MOUNTED__) {
	...
	let e = /* @__PURE__ */ new Map(), t = null, n = () => {
		...
		i.forEach((e) => {
			...
			if (i === -1) return;
			let a = ts(i), o = $.mvuDataMap.get(i), s = Object.keys(a).length === 0, c = o ? Object.keys(o).length === 0 : !0;
			!(s && c) && a !== o && $.mvuDataMap.set(i, a);
			...
			if (l.includes("pm-custom-char-mount-point") && !n.hasAttribute("data-setup-mounted")) {
				...
				if (!r.getElementById(a)) {
					...
					o.id = a, o.style.cssText = "margin-top: 15px; width: 100%; position: relative; clear: both;", n.appendChild(o), t = Xo(ip), t.mount(o), e.set(`setup_${i}`, t);
				}
			}
			if (!u && !d) {
				let t = n.querySelector(".pm-status-bar-container");
				if (!t && !e.has(i)) {
					t = r.createElement("div"), t.className = "pm-status-bar-container", t.id = `pm-app-${i}`, t.style.cssText = "margin-top: 15px; width: 100%; clear: both;", n.appendChild(t);
					let a = Xo(Wl, { mesId: i });
					a.mount(t), e.set(i, a);
				}
			}
