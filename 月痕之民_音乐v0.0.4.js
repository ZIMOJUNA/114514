...
//     import 'https://cdn.jsdelivr.net/gh/ZIMOJUNA/114514@main/月痕之民_音乐v0.0.4.js';
//
// ── v0.0.3 → v0.0.4 改了什么 ────────────────────────────────────────────────
//   驾驶员真机实测报回两条，**是两个互不相干的成因**：
//   ① **球拖不动，拖出来的是「球的图」** —— 球里那张 `<img>` 被浏览器**原生拖拽**接管了。
//      `<img>` 天生可拖，一按一移它就把这一下当成「拖图片」，`pointermove` 一个都收不到；
//      松手还会把图**投放到酒馆界面上**，弹「Unsupported file type: …音乐球.webp」
//      （名字是百分号编码的 ⇒ 来自 URL、不是本地文件）。
//      🔴 **这是 v0.0.3 加球图引进来的新病** —— v0.0.2 的球是手写 SVG，SVG 天生不可拖。
//      ⇒ 三处堵：`draggable="false"` ＋ `pointer-events:none` ＋ `-webkit-user-drag:none`。
//   ② **面板不跟着球走** —— 独立成因：拖拽里每步只挪球，**没让面板重新定位**，
//      面板只在「重新渲染」时定位一次，而拖着走的全程一次渲染都不跑 ⇒ 松手才「跳」过去。
//      ⇒ 拖动里加一句 `if (窗 && !窗.hidden) 定位窗();`
//   ⚠ 存储结构版本**仍是 2** —— 本轮一个字段都没加，纯行为修复。
//
// ── 它做什么 ────────────────────────────────────────────────────────────────
...
.ym-pic{position:absolute;inset:2px;width:calc(100% - 4px);height:calc(100% - 4px);
  ...
  /* 🔴 pointer-events:none 与 HTML 上的 draggable="false" 是**两条不同的路**，都要：
     前者让指针**根本落不到图上**（点击一律归球那个 button，结构上没得抢），
     后者是万一将来谁把这行删了、或某个浏览器不认它，还有声明兜着。
     另外 -webkit-user-drag:none 管 WebKit 那一支。 */
...
`;
  ...
  function 建UI() {
    ...
    // 🔴 `draggable="false"` 是**必须的**，不是保险 —— 浏览器对 `<img>` 天生就开拖拽，
    //   手指一按一移，它抢先把这一下当成「拖图片」，后面 `pointermove` 一个都收不到，
    //   球就**拖不动**，而且会留一个跟着鼠标走的**残影**（真机报障：「他拖的好像是球的图」）。
    //   v0.0.2 的球是手写 SVG 图标（SVG 天生不可拖），所以那时没这个病 —— 加球图才引进来的。
    球.innerHTML = '<span class="ym-halo" aria-hidden="true"></span>' +
      '<img class="ym-pic" data-ym=pic alt="" aria-hidden="true" draggable="false">' +
  ...
  function 绑事件(文档) {
    ...
    球.addEventListener('pointermove', (e) => {
      ...
      球.style.right = ''; 球.style.bottom = '';
      // 🔴 面板得**在这里**跟着挪 —— 不能指望「下次渲染时自然会对」。
      //   渲染只在数据变了才跑；拖着球走的全程一次都不会跑，
      //   于是面板**原地不动**，松手那一刻才「跳」过去（真机报障：「面板也不能随着球走」）。
      //   ⚠ 这一步会读 `球.getBoundingClientRect()`，也就顺带强制一次布局 ——
      //     上两行刚写完 `left/top`，读到的就是新位置，正是要的。
      if (窗 && !窗.hidden) 定位窗();
    });
    const 收尾 = (e, 落位) => {
      if (!按下) return;
